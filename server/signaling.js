...
function activeCount(room) { return [...room.peers].filter((peer) => !peer.pending).length; }
  ...
  socket.on('message', (raw) => {
    ...
    if (message.type === 'join') {
      ...
      const room = roomFor(roomId);
        if (activeCount(room) >= 6) { send(socket, { type: 'full', capacity: 6 }); socket.close(); return; }
        ...
        send(socket, { type: 'joined', count: activeCount(room), session: roomId, host: room.host === socket, hostPlayer: room.host?.player || 'l’hôte', pending: socket.pending, state: room.state });
        if (socket.pending) send(room.host, { type: 'participant-request', player: socket.player });
        else broadcast(room, { type: 'peer-joined', count: activeCount(room), player: socket.player }, socket);
      return;
    }
      if (!roomId) return;
      const room = roomFor(roomId);
      if (message.type === 'participant-response' && socket === room.host) {
        ...
        if (target) { target.pending = !message.accepted; send(target, { type: message.accepted ? 'accepted' : 'rejected', state: room.state }); if (message.accepted) { broadcast(room, { type: 'peer-joined', count: activeCount(room), player: target.player }, target); } else { room.peers.delete(target); target.close(); } }
        return;
      }
    ...
    socket.on('close', () => { if (!roomId) return; const room = roomFor(roomId); if (!room) return; room.peers.delete(socket); if (room.host === socket) room.host = room.peers.values().next().value || null; broadcast(room, { type: 'peer-left', count: activeCount(room) }); if (!room.peers.size) rooms.delete(roomId); });
});
